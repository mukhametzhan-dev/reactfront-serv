...
export const Appointments = () => {
  ...
  const fetchAppointments = async (patient_id: number) => {
    try {
      const response = await axios.get(
        `http://happymedkz.serveo.net/get_appointments_for_patient_with_id?patient_id=${patient_id}`
      );
  ...
  const handleCancelClick = async (appointmentId: number) => {
    ...
    try {
      const countResponse = await axios.get(`http://happymedkz.serveo.net/count_canceled_appointments?patient_id=${patientId}`);
  ...
  const handleCancel = async (appointmentId: number) => {
    try {
      const response = await axios.post(`http://happymedkz.serveo.net/cancel`, { appointment_id: appointmentId });
      if (response.status === 200) {
